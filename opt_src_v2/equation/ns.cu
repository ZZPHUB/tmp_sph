    ...
    if (index < numParticles)
    {
        ...
        for (int z = -1; z <= 1; z++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                if (newgridHash + 1 <= par.hash_max && newgridHash - 1 >= 0)
                {
                    startIndex = cellStart[newgridHash - 1];
                    endIndex = cellEnd[newgridHash + 1];
    ...
    if (index < numParticles)
    {
        ...
        for (int z = -1; z <= 1; z++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                if (gridHash + 1 <= par.hash_max && gridHash - 1 >= 0)
                {
                    startIndex = cellStart[gridHash - 1];
                    endIndex = cellEnd[gridHash + 1];
