...
void SPH_NS_simpleversion(float* sortedPos, float* sortedVel, float* sorteddensity, float* sortedpressure, int* sorted_particle_type, float* densitydt, float* Veldt, int* cellStart, int* cellEnd, int numParticles, int* particleHash, int timestep, float* dofv, float* rhop_sum, float* w_sum)
{
    ...
    //computeDeltaterm_acousticdamper_preD_simpleversion << < numBlocks, numThreads >> > (sortedPos, sortedVel, sorteddensity, sorted_particle_type, cellStart, cellEnd, numParticles, particleHash, dofv);

    //computeBoundaryD << < numBlocks, numThreads >> > (sortedPos, sorteddensity, sortedpressure, cellStart, cellEnd, rhop_sum, w_sum, numParticles, particleHash, sorted_particle_type);
    ...
    //computeDensitydtD_simpleversion << < numBlocks, numThreads >> > (sortedPos, sortedVel, sorteddensity, sorted_particle_type, densitydt, cellStart, cellEnd, numParticles, particleHash);

    //computeVelocitydtD_simpleversion << < numBlocks, numThreads >> > (sortedPos, sortedVel, sortedpressure, sorteddensity, sorted_particle_type, dofv, Veldt, cellStart, cellEnd, numParticles, particleHash);
