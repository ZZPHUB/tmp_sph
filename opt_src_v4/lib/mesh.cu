...
__global__ void reorderD(int* particleHash, int* particleIndex, int* cellStart, int* cellEnd, float* oldPos, float* oldVel, float* olddensity, float* oldpressure, int* oldparticle_type, int* oldparticle_zone, float* sortedPos, float* sortedVel, float* sorteddensity, float* sortedpressure, int* sortedparticle_type, int* sortedparticle_zone,int *table_l,int *table_r)
{
    ...
    if (index < par.numParticles)
    {
       ...
       if(hash != sharedHash[ threadIdx.x ])
       {
            ...
            writesize = hash - sharedHash[threadIdx.x] - 1;
            //mymemset<<<32,32>>>(&(cellStart[sharedHash[threadIdx.x]+1]),&(cellEnd[sharedHash[threadIdx.x]+1]),writesize,index);
            //zzpmemset<<<(writesize+32)/32,32>>>(&(cellStart[sharedHash[threadIdx.x]+1]),&(cellEnd[sharedHash[threadIdx.x]+1]),writesize,index);
            /*
...
__global__ void fill_cell(int *cell_start,int *cell_end,int *hash)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    int next_cell_start = tid;
    int next_cell_end = tid;
    if(tid < par.numCells)
    {
        if(cell_start[tid] == 0xffffffff)
        {
            if(tid < hash[0])
            {
                cell_start[tid] = 0;
            ...
            else if (tid > hash[par.numParticles-1])
            {
                cell_start[tid] = par.numParticles-1;
                cell_end[tid] = par.numParticles-1;
            }
            else
            {
                next_cell_start += 1;
                next_cell_end -= 1;
                while (next_cell_start < par.numCells && cell_start[next_cell_start]==0xffffffff && cell_start[next_cell_end]==0xffffffff )
                {
                    next_cell_start += 1;
                    next_cell_end -= 1;
                }
                if(next_cell_start < par.numCells)
                {
                    if(cell_start[next_cell_start] != 0xffffffff) cell_start[tid] = cell_end[tid] = cell_start[next_cell_start];
                    else cell_start[tid] = cell_end[tid] = cell_end[next_cell_end];
                }
            }           
        }
    }
...
void reorder(float* oldPos, float* oldVel, float* olddensity, float* oldpressure, int* oldparticle_type, int* oldparticle_zone, float* sortedPos, float* sortedVel, float* sorteddensity, float* sortedpressure, int* sortedparticle_type, int* sortedparticle_zone, int* particleHash, int* particleIndex, int* dcellStart, int* dCellEnd, int numParticles, SimParams* temps,int *table_l,int *table_r)
{
    ...
    fill_cell<<<(int)((temps->numCells+256)/256),256>>>(dcellStart,dCellEnd,particleHash);
    cudaDeviceSynchronize();
    cudaError_t cudaStatus = cudaGetLastError();
    if (cudaStatus != cudaSuccess)
    {
