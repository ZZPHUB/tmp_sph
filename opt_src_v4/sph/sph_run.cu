...
int main(void)
{
    ...
    temps->gridSize = temps->particleR;
    ...
    //for calculation
    temps->h2 = temps->h*temps->h;
    temps->cs2 = temps->cs*temps->cs;
    temps->eta2 = temps->eta*temps->eta;
    temps->delta_h_cs = temps->delta*temps->h*temps->cs;
    temps->h_cs_rho_m = temps->h * temps->cs * temps->restDensity * temps->particleMass;
    ...
    {
        std::cout << "SPH_NS launch failed: " << cudaGetErrorString(cudaStatus) << std::endl;
        system("pause");
    }
    ///constant variables set--------------------------------------------------------------
    ...
    for (int i = 0; i < ngpus; i++)
    {
        ...
        std::cout << "Device " << i << " " << devProp.name << " has compute capability " << devProp.major << "." << devProp.minor << std::endl;
    }
    cudaSetDevice(0);
    cudaDeviceProp devProp;
    cudaGetDeviceProperties(&devProp, 0);
    std::cout << "Device 0 is used! " << std::endl;
   ...
   // float4* h_r_m = new float4[np];
    ...
    int thread_num = 1;
    std::vector<float *> buff_pos(thread_num);
    std::vector<float *> buff_vel(thread_num);
    std::vector<float *> buff_p(thread_num);
    std::vector<int *> buff_table(thread_num);
    std::vector<int *> buff_type(thread_num);
    std::atomic<int> buff_flag[thread_num];
    int buff_frame[thread_num];
    ...
    {
        buff_pos[i] = new float[np*3];
        buff_vel[i] = new float[np*3];
        buff_p[i] = new float[np*3];
        buff_table[i] = new int[np];
        buff_type[i] = new int[np];
        buff_flag[i] = 0;
        buff_frame[i] = 0;
        thread_pool.emplace_back(thread_loop,&buff_frame[i],np,dt,dx,buff_pos[i],buff_vel[i],buff_p[i],buff_table[i],buff_type[i],measuring_particle,&buff_flag[i]);
    ...
    std::cout << "计算开始时间：" << date_time_start << std::endl;

    ofile.open("./result/Simulation records.dat", ios::out);
    ofile << "This file records the simulation instants!" << std::endl;
    ofile << "计算开始时间：" << date_time_start << std::endl;
    ofile.close();
        ...
        if (((i + 1) % out_putstep) == 1)
        {
            ...
            while (skip_flag)
            {
                for(int j=0;j<thread_num;j++)
                {
                    ref_0 = 0;
                    ref_2 = 2;
                    if(buff_flag[j].compare_exchange_strong(ref_0,2))
                    {
                        //memcpy
                        cudaMemcpy(buff_table[j],d_table_r,sizebyte_ui,cudaMemcpyDeviceToHost);
                        cudaMemcpy(buff_pos[j], d_pos, sizebyte_f2, cudaMemcpyDeviceToHost);
                        cudaMemcpy(buff_vel[j], d_vel, sizebyte_f2, cudaMemcpyDeviceToHost);
                        //cudaMemcpy(rhop, d_density, sizebyte_f1, cudaMemcpyDeviceToHost);
                        cudaMemcpy(buff_p[j], d_pressure, sizebyte_f1, cudaMemcpyDeviceToHost);
                        cudaMemcpy(buff_type[j], d_particle_type, sizebyte_ui, cudaMemcpyDeviceToHost);
                        buff_frame[j] = i;
                        //cudaMemcpy();
                        now = time(0);
                        date_now = ctime(&now);
                        std::cout << "Time step " << i << ",    Simulation time " << i * dt << "s,   Real time: " << date_now << std::endl;

                        ofile.open("./result/Simulation records.dat", ios::out | ios::app);
                        ofile << "Time step " << i << ",    Simulation time " << i * dt << "s,   Real time: " << date_now << std::endl;
                        ofile.close();
                        
                        //thread synchronize 
                        buff_flag[j].compare_exchange_strong(ref_2,1);
                        skip_flag = false;
                        break;
                    }
                } 
